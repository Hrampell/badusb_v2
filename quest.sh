...
# --- Trigger a Jumpscare in Safari with Full-Screen Resize ---
def trigger_jumpscare(video_url)
  ...
  # Wait briefly to allow the video to start playing.
  sleep 0.5
  
  # Use AppleScript to get the desktop bounds from Finder and set Safari's front window to them.
  # This effectively resizes the Safari window to full screen.
  system(<<-APPLE
osascript -e 'tell application "Finder" to set theBounds to bounds of window of desktop' -e 'tell application "Safari" to set bounds of front window to theBounds'
APPLE
  )
  
  # Unhide Safari's front window.
